...
# Configurações
OUTPUT_SIZE = 360   # tamnho da janela
...
# divisão
print("\nSeparando Treino e Teste (Priorizando Teste = 400)")
...
for class_id in range(5):
    ...
    if total_class >= TARGET_TEST_FIXED:
        cutoff = TARGET_TEST_FIXED
    else:
        cutoff = int(total_class * 0.2)
        
    print(f"Classe {class_id} (Total: {total_class}): {cutoff} para Teste | {total_class - cutoff} para Treino Base")

    # Fatia os dados
    # TESTE: Pega os primeiros cutoff
    X_test_ext.extend(X_class_all[:cutoff])
    y_test.extend(y_class_all[:cutoff])
    
    # TREINO: Pega o resto
    X_train_ext.extend(X_class_all[cutoff:])
    y_train.extend(y_class_all[cutoff:])
...
for class_id in range(5):
    ...
    if n_samples > TARGET_TRAIN:
        ...
        # 2. Gera novos sintéticos até completar 1600
